...
// ---
const std::vector <QGAMES::MazeModel::PositionInMaze>& PacManII::Maze::positionsForZone (int n) const
{
...
// ---
bool PacManII::Maze::arePositionsTogether 
...
// ---
std::vector <QGAMES::Vector> PacManII::Maze::restrictedDirectionsAt (const QGAMES::MazeModel::PositionInMaze& p) const
{
	std::vector <QGAMES::Vector> result;

	QGAMES::SetOfOpenValues sMDirs;
	if (mazeInfoAt (p).existSetOfOpenValues (__PACMANII_MAZEINFODIRZONE__))
		sMDirs = mazeInfoAt (p).setOfOpenValues (__PACMANII_MAZEINFODIRZONE__);
...
PacManII::Maze PacManII::Maze::generateMazeFrom (int x, int y, const PacManII::Map* mp)
{
	...
	std::vector <std::vector <QGAMES::MazeModel::PositionInMaze>> znes 
		(__PACMANII_MAZENUMBERPOSSIBLEZONES__, std::vector <QGAMES::MazeModel::PositionInMaze> ());
	for (auto i : mp -> mazeZonesLayer () -> tiles ())
	{
		...
		if (pNZ != -1 && pNZ < __PACMANII_MAZENUMBERPOSSIBLEZONES__)
	...
	static std::vector <int> hA = __PACMANII_MAZEHORIZONTALMOVEMENTZONES__;
	static std::vector <int> vA = __PACMANII_MAZEVERTICALMOVEMENTZONES__;
	for (int i = 0; i < __PACMANII_MAZENUMBERPOSSIBLEZONES__; i++)
	{
		for (auto j : znes [i])
		{
			// The information about the zone the position belogs to is added to the map...
			QGAMES::SetOfOpenValues mI = result.mazeInfoAt (j);
			mI.addOpenValue (__PACMANII_MAZEINFOZONE__, QGAMES::OpenValue (i));
			...
			if (mM [0] != QGAMES::Vector::_noPoint)
			{
				...
				if (result.isPossibleToMoveTo (j, mM [0]) && result.isPossibleToMoveTo (j, mM [1]))
				{
					...
					if (!aDirs.empty ())
					{
						...
						mI.addSetOfOpenValues (__PACMANII_MAZEINFODIRZONE__, Rdrs); 
					}
				}
			}
	...
	for (auto i : tunnelPos)
	{
		...
		mI1.addOpenValue (__PACMANII_MAZEINFOTUNNELCONNECTION__, 
		...
		mI2.addOpenValue (__PACMANII_MAZEINFOTUNNELCONNECTION__, 
...
std::vector <QGAMES::Vector> PacManII::Maze::allPossibleDirectionsAt (const PositionInMaze& p) const
{
	...
	if (result.size () == 1 && mI.existOpenValue (__PACMANII_MAZEINFOTUNNELCONNECTION__))
	...
	(const QGAMES::MazeModel::PositionInMaze& p, const QGAMES::Vector& d) const
{
	...
	{
		QGAMES::SetOfOpenValues mI = mazeInfoAt (p);
		if (mI.existOpenValue (__PACMANII_MAZEINFOTUNNELCONNECTION__)) // ...it could be a tunnel, if it is defined as it!
			result = QGAMES::MazeModel::PositionInMaze 
				(QGAMES::Vector (mI.openValue (__PACMANII_MAZEINFOTUNNELCONNECTION__).strValue ()));
	}
	else
	{
